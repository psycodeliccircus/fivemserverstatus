...
exports.start = function(SETUP) {
  ...
  const UpdateEmbed = function() {
    ...
    .setAuthor("AGPA RP Server Status", "https://cdn.discordapp.com/attachments/620714726195724299/626580331654545418/logo1.png")
    .setColor(0x2894C2)
    .setFooter(TICK_N % 2 === 0 ? '⚪ AGPARP' : '⚫ AGPARP')
    .setTimestamp(new Date())
    .addField('\n\u200b\nComo você pode entrar no servidor?','Você pode entrar no servidor através do site **www.agrandeportoalegre.com** para ser inserido na Whitilist.\n Desenvolvido por [ESXBrasil](https://www.esxbrasil.online/)\n\u200b\n',false)
    if (STATUS !== undefined)
    {
      embed.addField(':warning: Status atual do servidor:',`${STATUS}\n\u200b\n`);
  ...
  const updateMessage = function() {
    getVars().then((vars) => {
      getPlayers().then((players) => {
        ...
        .addField('Fila',queue === 'Enabled' || queue === undefined ? '0' : queue.split(':')[1].trim(),true)
        .addField('Jogadores online',`${players.length}/${MAX_PLAYERS}\n\u200b\n`,true);
        // .addField('\u200b','\u200b\n\u200b\n',true);
        if (players.length > 0) {
          // method D
          const fieldCount = 3;
  ...
  bot.on('ready',() => {
    ...
    // bot.user.setGame('RenildoMarcio', 'https://www.twitch.tv/renildomarcio');
    bot.user.setActivity('AGPA RP',{'url':'https://www.twitch.tv/renildomarcio','type':'STREAMING'});
      ...
      if (message.member) {
        if (message.member.hasPermission('ADMINISTRATOR')) {
          if (message.content.startsWith('+status ')) {
            ...
            .setColor(0x2894C2)
            .setTitle('Mensagem de status atualizada')
            .setTimestamp(new Date());
            if (status === 'clear') {
              STATUS = undefined;
              embed.setDescription('Mensagem de status limpa');
            } else {
              STATUS = status;
              embed.setDescription(`Nova mensagem:\n\`\`\`${STATUS}\`\`\``);
            }
        ...
        if (message.channel.id === SUGGESTION_CHANNEL) {
          ...
          message.channel.send(embed).then((message) => {
            ...
            sent.react('👍').then(() => {
              sent.react('👎').then(() => {
                log(LOG_LEVELS.SPAM,'Mensagem de sugestão concluída');
              }).catch(console.error);
            }).catch(console.error);
          }).catch(console.error);
        ...
        if (message.channel.id === BUG_CHANNEL) {
          ...
          .setColor(0x2894C2)
          .setTitle('Bug Report')
          .setDescription('Sua mensagem foi enviada com sucesso para a equipe!')
          .setTimestamp(new Date());
  ...
  bot.login(BOT_TOKEN).then(null).catch(() => {
    log(LOG_LEVELS.ERROR,'Não foi possível fazer o login, verifique seu token de login');
    console.error(e);
    process.exit(1);
  });
